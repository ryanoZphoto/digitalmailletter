...
async function processJob(jobId: string) {
  ...
  const tracking = await lob.sendLetterPDF({ to, from, pdfBuffer: pdf, description: (job.templateId ?? undefined) as any });
    ...
    await prisma.job.update({ where: { id: jobId }, data: { status: 'sent', tracking: tracking as any } });
  } catch (err) {
  ...
  await prisma.job.update({ where: { id: jobId }, data: { status: nextStatus, options: { ...currentOptions, attempts } as any } });
...
async function pollLoop() {
  ...
  const jobs = await prisma.job.findMany({ where: { status: 'submitted' }, take: WORKER_CONCURRENCY });
  for (const j of jobs) {
...
if (process.env.NODE_ENV !== 'test') {
  console.log('Worker starting, connecting to DB...');
  prisma.$connect().then(() => {
